		  ...
		  map<int, vector<pair<pg_notify_t, pg_interval_map_t> > > *activator_map)
{
  ...
  if (is_primary()) {
	...
	state_set(PG_STATE_RESERVE_BACKFILL);
      else
	active++;
...
void PG::all_activated_and_committed()
{
  ...
  if (info.last_complete == info.last_update &&
      !state_test(PG_STATE_RESERVE_BACKFILL) &&
...
void PG::finish_recovery(ObjectStore::Transaction& t, list<Context*>& tfin)
{
  ...
  state_clear(PG_STATE_RESERVE_BACKFILL);
...
PG::RecoveryState::WaitRemoteBackfillReserved::react(const RemoteReservationRejected &evt)
{
  PG *pg = context< RecoveryMachine >().pg;
  pg->osd->local_reserver.cancel_reservation(pg->info.pgid);
  pg->state_clear(PG_STATE_RESERVE_BACKFILL);
  ...
  : my_base(ctx)
{
  ...
  int64_t max = kb * g_conf->osd_refuse_backfill_full_ratio;
  if (kb_used >= max) {
